...
var Magnacarto = function (config) {
    ...
    switch (process.platform) {
        case 'linux':
            library = 'libmagnacarto.so';
            break;
        case 'darwin':
            library = 'libmagnacarto.dylib';
            break;
        default:
            throw new Error('Your platform is currently not supported.');
            break;
    }

    this.lib = ffi.Library(path.normalize(path.join(__dirname, '..', 'magnacarto', 'bin', library)), {
...
Magnacarto.prototype.buildFromFile = function (mmlfile) {
    ...
    var output = this.lib.buildFromFile(path.resolve(mmlfile), this.getOpts());
    if (output.err != null) {
        throw output.err;
    }
...
Magnacarto.prototype.buildFromString = function (mml) {
    ...
    var output = this.lib.buildFromString(mml, this.getOpts());
    if (output.err != null) {
